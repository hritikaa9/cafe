...
const getProduct = async(req, res) =>{
    try{
        const query = "SELECT p.id, p.name, p.description, p.price, p.status, c.id as categoryId, c.name as categoryName FROM product as p INNER JOIN category as c ON p.category_id = c.id ";
...
const getProductById = async(req, res) =>{
    try{
        const id = req.params.id;
        const query = "SELECT p.id, p.name, p.description, p.price, c.id as categoryid, c.name AS categoryname FROM product p INNER JOIN category c ON p.category_id = c.id where p.id = $1";
        const values = [id];
        await client.query(query, values, (err,result) => {
            if (!err){
                return res.status(200).json(result.rows);
